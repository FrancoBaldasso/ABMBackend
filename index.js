...
app.post("/", async function (request, response) {
  try {
    const { nombre, departmento, direccion } = request.body;
    const [result] = await addEmployee(nombre, departmento, direccion);
    if (result.insertId) {
...
app.put("/:id", async function (request, response) {
  try {
    ...
    const { id } = request.params;
    const [result] = await updateAddress(id, direccion);
